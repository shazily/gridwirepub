...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Allow join by organization ID — when enabled, authenticated users can self-join as Viewer via the org UUID form (onboarding) or a shareable /join/{portal_slug|uuid} link. Off by default to prevent accidental org discovery. Use invite links for Contributor+.",
  ...
  },
  {
    ...
    bullets: [
      ...
      "Invite links — create_org_invite RPC; share /invite/{token}. Roles for invites: contributor, member, viewer. Prefer invites when you need more than Viewer.",
      "Join by org ID — enable under Authentication; joiners use the org UUID on onboarding or /join/{slug}. Role is always Viewer until an admin promotes them.",
  ...
  {
    id: "join-by-org-id",
    title: "Join a workspace without an invite (org UUID / join link)",
    category: "Email & authentication",
    keywords: ["join", "org id", "uuid", "viewer", "on-prem", "onboarding", "allow_join_by_org_id"],
    paragraphs: [
      "On self-hosted instances (marketing homepage off), not everyone creates an organization. After sign-up, onboarding asks you to join with an organization UUID, paste an invite link, or (secondary) create the first workspace.",
      "Admins enable “Allow join by organization ID” under Authentication, then share either the organization UUID or the copyable /join/{portal_slug} link. Links may use portal slug or UUID; the onboarding form accepts UUID only.",
      "Successful join calls join_organization_by_ref and adds the user as Viewer (external / local). Failures (unknown id, join disabled, disabled member) all return the same generic message so orgs cannot be discovered by guessing.",
    ],
    bullets: [
      "Gate: organizations.auth_config.allow_join_by_org_id must be true.",
      "Preview RPC get_join_preview is service_role only (used by the /join page server function).",
      "Pending join refs are stored in localStorage (gridwire.pendingJoinRef) and redeemed on first dashboard load, similar to invites.",
      "Invite links remain the path for Contributor, Member, or custom roles.",
    ],
    links: [
      { to: "/admin/authentication", label: "Enable join (Authentication)" },
      { to: "/admin/team", label: "Team & access (invites)" },
      { to: "/onboarding", label: "Onboarding / join" },
    ],
  },
  {
    id: "marketing-signup",
    ...
    bullets: [
      ...
      "Self-host / on-prem builds (VITE_SHOW_MARKETING off) use a different /auth layout focused on joining an existing workspace, not the marketing hero.",
    ],
  ...
  },
  {
    ...
    bullets: [
      "Status bar — Activate / Pause ingest for the whole workspace (confirm overlay). Checklist: address, senders, template, activated.",
      "Tabbed setup: Mailbox (primary address + aliases, Save mailbox settings) → Senders → Templates → Notifications → Test & log.",
      "Every add/save opens a summary overlay: Confirm, Edit, or Discard.",
      "Ingest templates (not just columns) — click to view/edit in an overlay; activate or deactivate individually.",
      "Aliases: extra To addresses that route to the same workspace pipeline (same senders/templates/activation). Your gateway must deliver those addresses; Gridwire matches them on inbound webhook.",
      "Webhook URL must be a public HTTPS URL your mail gateway can reach — not 127.0.0.1. Set Authentication → Public app URL and PUBLIC_APP_URL.",
      "Ingest address must use a real mail domain (not @ingest.local). Set INGEST_EMAIL_DOMAIN for suggestions.",
      "Notification recipients (success/failure per email) — platform mailer. Rejected senders get an automatic rejection email when mail reaches the webhook.",
      "Dashboard and Admin home show live ingest status, today’s counts, recent messages, and ops alerts.",
      "Test ingest and recent messages log (portal test bypasses the external gateway).",
    ],
  ...
  },
  {
    ...
    bullets: [
      ...
      "API keys / API docs / Connectors / Logs / Usage — as labeled in the sidebar.",
    ],
  ...
  {
    id: "audit-usage",
    title: "Logs, audit and usage",
    category: "Admin console",
    keywords: ["compliance", "csv", "consumption", "logs", "audit", "email ingest"],
    paragraphs: [
      "Logs consolidates email ingest history, system/ops alerts, and the insert-only audit trail. Usage analytics remain under Admin → Usage. The dashboard only shows live counts and alert summaries.",
    ],
    bullets: [
      "Logs → Email ingest — inbound message statuses and rejection reasons (owners/admins).",
      "Logs → System & ops — ingest outcomes, suspicious API access, connector failures.",
      "Logs → Audit — control-plane and data-access events with CSV export (owners/admins).",
    ],
    links: [
      { to: "/logs", label: "Open Logs" },
...
export const HELP_FAQS: { q: string; a: string; keywords?: string[] }[] = [
  ...
  },
  {
    ...
    a: "Admin → Team & access: invite links and/or Add local user. For Viewer self-join without an invite (on-prem), enable Authentication → Allow join by organization ID and share the org UUID or /join link.",
    keywords: ["invite", "team", "join"],
  },
  {
    q: "How do I join an existing workspace without an invite?",
    a: "Ask an admin to enable join by organization ID, then use the shared /join link or enter the organization UUID on onboarding after you create an account. You join as a Viewer.",
    keywords: ["join", "uuid", "viewer", "on-prem"],
  },
];
